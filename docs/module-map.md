...
| `src/scheduler/mod.rs` | `SchedulerRunner` struct, poll loop, `tick()` — scheduled redispatch for tasks with `schedule_interval_secs` set. Structurally parallel to `FeedRunner`, started beside it. Skips dispatching entirely when a pinned branch has not moved since `last_processed_sha` |
