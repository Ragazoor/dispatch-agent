...
| `src/agent_tree_open_set.rs` | The one-way channel between the two panes: which files' diffs are open. The tree writes, the diff pane reads, never the reverse. Every read soft-fails to "nothing is open", and a recorded path that escapes the worktree is dropped rather than handed to git |
