...
But `path::symbol` is **only partly** verified, so don't treat it as self-maintaining. `check-doc-symbols.sh` rejects **backticked snake_case** identifiers that occur nowhere in the code — which is a real net, but it leaves two rots green. A **PascalCase** symbol that has been deleted (`FeedJob`, after its struct is gone) is not snake_case, so nothing flags it. And an **unbackticked** citation inside an Allium `@guidance` block is not backticked, so a spec written spec-first can cite a function name the code never ends up using — #4091 specified `src/feed/cycle.rs::run_feed_cycle` <!-- allow-phantom-symbol: naming the exact rot this paragraph warns about; the real symbol is FeedCycle::run --> and shipped as `FeedCycle::run` through two commits. So: when you rename or delete a type, `grep` the old name across `CLAUDE.md`, `docs/` and `src/**/*.rs` doc comments; and when a spec lands before its code, re-check its citations against the finished code before pushing.
...
- [docs/conventions.md](docs/conventions.md) — `FieldUpdate`, `TaskPatch`/`EpicPatch` double-Option, DB trait narrowing, `db_call`/`db_call_read` (writer + read-pool model), rendering-purity panic policy, service mutation boundary, `recalculate_epic_status` invariant, inline-mutation boundary, `LearningService` injection state, `let _`, dead code, sub-status TOCTOU, epic reparenting guards, Clippy, visibility, performance footguns (`column_items_for_status` test-only; no `std::fs` in async), the one bounded-child primitive (`run_bounded` — never hand-roll a second kill-on-timeout), keybinding telemetry (every key arm records; tests asserting exact command lists need `without_usage`), prod-vs-test LOC split
