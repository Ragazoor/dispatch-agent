...
A `file:NN` citation is only **bounds-checked**: `check-doc-paths.sh` confirms the file has at least that many lines, never that the line still holds what the doc says. Inserting lines into a cited file silently shifts every citation below the insertion point and the hook stays green — prefer the `path::symbol` form (`src/feed/exec.rs::exec_feed_command`), which does not rot.
