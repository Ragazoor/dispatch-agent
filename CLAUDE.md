...
The pre-push hook (`.githooks/pre-push`) runs, in order: `cargo fmt` (auto-formats), `cargo clippy --all-targets -- -D warnings` (no `--fix` — it checks, it does not rewrite), `./scripts/check-doc-paths.sh` (validates every `src/…`/`docs/…` path and `file:NN` line citation in `CLAUDE.md`, every `docs/*.md`, and every `docs/specs/*.allium` — globbed, so a new doc is covered the moment it lands; the dated artifacts under `docs/plans/`, `docs/superpowers/`, and `docs/research/` are excluded), `bash ./scripts/test-check-doc-paths.sh` (self-test for that checker), `./scripts/check-doc-symbols.sh` (rejects symbol citations in the agent-facing docs and in `src/**/*.rs` doc comments that no longer resolve — backticked snake_case identifiers, `path.rs::symbol` citations, `Type::method` citations, and bare snake_case names of five or more words, backticked or not; annotate a deliberate reference to removed or external code with `allow-phantom-symbol: <why>`, which must sit **on the citing line or the one directly above it** — two lines above does not count), `bash ./scripts/test-check-doc-symbols.sh` (self-test for that checker), `./scripts/check-no-test-sleep.sh` (rejects `tokio::time::sleep` anywhere under `src/`/`tests/`, and `std::thread::sleep` in test files — see the async-test rule below), and `bash ./scripts/test-fetch-reviews.sh` (stub-`gh` test for the review feed script). Run `cargo test` separately before pushing.
